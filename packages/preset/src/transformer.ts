import ts, { type TransformationContext, type Transformer } from "typescript";
...
export function visitStatements(
	...
	const mapped = [...stmts].flatMap((stmt) => {
		if (ts.isImportDeclaration(stmt)) {
			const ext = Path.extname((stmt.moduleSpecifier as ts.StringLiteral).text);

			if (
				!EXT.includes(ext) ||
				!(stmt.moduleSpecifier as ts.StringLiteral).text.startsWith(".")
			)
				return stmt;
			...
			if (exports) {
				...
				if (!ident) throw new Error("only support default or namespace import");
...
export function transform(
	...
	return (ctx: TransformationContext): Transformer<ts.SourceFile> =>
		(node: ts.SourceFile) => {
